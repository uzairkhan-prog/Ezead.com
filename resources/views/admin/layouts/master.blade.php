...
<script>
    $(document).ready(function() {
        $(document).on('click', '.custom-ajax-request', function(e) {
            e.preventDefault();
            var $this = $(this);
            var id = $this.data('id');
            var value = $this.data('value');
            var $toggleIcon = $this.find('.admin-single-icon');
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        var $installButton = $('#install-' + id);
                        if (response.new_value === '1') {
                            $toggleIcon.removeClass('fa-toggle-off toggle-off').addClass(
                                'fa-toggle-on toggle-on');
                            $installButton.html('<i class="fas fa-download"></i> Installed')
                                .removeClass('btn-light').addClass('btn-success');
                        } else {
                            $toggleIcon.removeClass('fa-toggle-on toggle-on').addClass(
                                'fa-toggle-off toggle-off');
                            $installButton.html('<i class="fas fa-download"></i> Install')
                                .removeClass('btn-success').addClass('btn-light');
                        }
                        $this.data('value', response.new_value);
                    }
                },
