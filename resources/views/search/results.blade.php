...
<div class="main-container">
    ...
    <input type="hidden" value="{{ Route::currentRouteName() }}" class="searchPageURL">
    ...
    @includeFirst([
    config('larapen.core.customizedViewPath') . 'search.inc.breadcrumbs',
    'search.inc.breadcrumbs',
    ])

    @if (config('settings.list.show_cats_in_top'))
    @if (isset($cats) && !empty($cats))
    <div class="container mb-2 hide-xs px-4">
    ...
    @endif
    @includeFirst([
    config('larapen.core.customizedViewPath') . 'search.inc.categories',
    'search.inc.categories',
    ])
    @endif
            ...
            <div class="{{ $contentColSm }} page-content col-thin-left mb-4">
                <div class="category-list {{ config('settings.list.display_mode', 'make-grid') }}{{ $contentColSm == 'col-md-12' ? ' noSideBar' : '' }}">
                    <div class="tab-box">
                        ...
                        {{-- Nav tabs --}}
                        <ul id="postType" class="p-2" role="tablist">
                            @php
                            $aClass = '';
                            ...
                            @endphp
                            <li class="nav-item">
                                <a href="{!! request()->fullUrlWithoutQuery(['page', 'type']) !!}" class="nav-link{{ $aClass }} custom-search-nav-link">
                                    {{ t('All Listings') }} <span class="badge badge-pill {!! $spanClass !!}  custom-bg-danger">{{ data_get($count, '0') }}</span>
                                </a>
                            </li>
                            @if (config('settings.single.show_listing_types'))
                    ...
                    <div class="listing-filter d-none">
                    ...
                    <div class="tab-content" id="myTabContent">
                        <div class="tab-pane fade show active" id="contentAll" role="tabpanel" aria-labelledby="tabAll">
                            <div id="postsList" class="category-list-wrapper posts-wrapper row no-margin border">
                    ...
                    <div class="tab-box save-search-bar text-center">
                        <a id="saveSearch" data-name="{!! request()->fullUrlWithoutQuery(['_token', 'location']) !!}" data-count="{{ data_get($count, '0') }}">
    ...
    <div class="container mb-3">
        <div class="card text-dark custom-bg-light mb-3 p-3 border">
            <div class="card-body text-center">
                ...
                <a href="#quickLogin" class="btn btn-border btn-post btn-listing shadow" data-bs-toggle="modal">{{ t('start_now') }}</a>
                @else
                <a href="{{ \App\Helpers\UrlGen::addPost() }}" class="btn btn-border btn-post btn-listing shadow">{{ t('start_now') }}</a>
                @endif
            </div>
        </div>
    </div>
