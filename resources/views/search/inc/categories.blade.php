@if ((isset($cat) && !empty($cat)) || (isset($cats) && !empty($cats)))
    <div class="container mb-3 hide-xs">
        @if (isset($cat) && !empty($cat))
            ...
            @php
                $AcsOrderCategories = collect(data_get($cat, 'children'))->sortBy('name');
            @endphp
                <div class="row row-cols-lg-4 row-cols-md-3 p-2 g-2" id="categoryBadge">
                    @foreach ($AcsOrderCategories as $iSubCat)
                        <div class="col">
            ...
            @else
                ...
                @php
                    $AscOrderSubcategory = collect(data_get($cat, 'parent.children'))->sortBy('name');
                @endphp
                    <div class="row row-cols-lg-4 row-cols-md-3 p-2 g-2" id="categoryBadge">
                        @foreach ($AscOrderSubcategory as $iSubCat)
                            <div class="col">
                ...
                @else
                    @includeFirst([
                        config('larapen.core.customizedViewPath') . 'search.inc.categories-root',
                        'search.inc.categories-root',
                    ])
