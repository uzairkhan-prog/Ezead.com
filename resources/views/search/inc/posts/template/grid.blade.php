...
@if (!empty($posts) && $totalPosts > 0)
	@foreach($posts as $key => $post)
        @php
            // Custom GET All Locations
            // Get Province
            $post_province_id = DB::table('posts')
                ->where('id', $post['id'])
                ->value('province_id');
            $province = DB::table('regions')
                ->where('id', $post_province_id)
                ->value('name');
            // Get Region
            $post_region_id = DB::table('posts')
                ->where('id', $post['id'])
                ->value('region_id');
            $region = DB::table('regions')
                ->where('id', $post_region_id)
                ->value('name');
            // Get New-City
            $post_new_city_id = DB::table('posts')
                ->where('id', $post['id'])
                ->value('new_city_id');
            $new_city = DB::table('regions')
                ->where('id', $post_new_city_id)
                ->value('name');
            // Get Neighbour
            $post_neighbour_id = DB::table('posts')
                ->where('id', $post['id'])
                ->value('neighbour_id');
            $neighbour = DB::table('regions')
                ->where('id', $post_neighbour_id)
                ->value('name');
        @endphp
		<div class="item-list">
					...
					<div class="items-details">
						...
						<span class="info-row">
                            ...
                            {{-- <span class="item-location"{!! config('lang.direction') == 'rtl' ? ' dir="rtl"' : '' !!}>
                                ...
                                {{ !empty(data_get($post, 'distance')) ? '- ' . round(data_get($post, 'distance'), 2) . getDistanceUnit() : '' }}
                            </span> --}}
