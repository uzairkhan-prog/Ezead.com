...
<div class="modal fade modalHasList" id="selectCountry" tabindex="-1" aria-labelledby="selectCountryLabel"
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <style>
                .custom-image-header-modal img.flag.flag-lp {
                    background-position: -50px -163px !important;
                }
            </style>
                ...
                <div class="row row-cols-lg-4 row-cols-md-3 row-cols-sm-2 row-cols-2">
                    ...
                    @if (isset($countries))
                        @foreach ($countries as $code => $country)
                            <div class="col mb-1 cat-list custom-image-header-modal">
                                @php
                                    // $url = url('/?country=' . str($country->get('code')));
                                ...
                                <img src="{{ str($country->get('flag_url')) }}"
                                    class="flag flag-{{ $country->get('icode') }}"
                                    style="margin-bottom: 4px; margin-right: 5px;">
                                <a href="#" data-bs-toggle="tooltip" data-bs-custom-class="modal-tooltip"
                                    title="{{ $country->get('name') }}" code="{{ str($country->get('code')) }}"
