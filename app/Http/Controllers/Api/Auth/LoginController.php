...
class LoginController extends BaseController
{
	...
	{
	    $emailErrorMessage = 'E-mail does not exist';
		...
		try {
		    if($request->u && $request->r){
    			...
    			$user = User::where($dbField, $fieldValue)->first();

				if (!$user) {
					return $this->respondError($emailErrorMessage);
				}
    			$credentials = [
    				$dbField   => $fieldValue,
			    ...
			    if(auth()->attempt($credentials, $request->has('remember_me'))){
					...
					return $this->respondError($passwordErrorMessage);
				};
		    }
