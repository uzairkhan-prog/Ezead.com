...
trait SearchTrait
{
	...
	protected function normalQuery()
	{
		...
		$areBelongLoggedUser = ((request()->filled('belongLoggedUser') && request()->integer('belongLoggedUser') == 1) || request()->get('logged'));
		...
		$posts = Post::query()->with(['user'])->whereHas('country');

		if (!in_array(request()->getPathInfo(), ['/account/posts/list'])) {
			$posts->countryOf($countryCode);
		}
