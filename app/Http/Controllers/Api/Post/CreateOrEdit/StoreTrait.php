...
trait StoreTrait
{
	...
	public function storePost(PostRequest $request)
	{
        ...
        // Start Custom Add If Tags ?? Category
        if ($request->input('tags') === null || $request->input('tags') === '') {
            $postCatID = $request->input('category_id');
            $postCat = Category::find($postCatID);
            $postCatName = str_replace('/', 'and', $postCat->name);
            $post->tags = $postCatName;
        }
        // End Custom Add If Tags ?? Category
