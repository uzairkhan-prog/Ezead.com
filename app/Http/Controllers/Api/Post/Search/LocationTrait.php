...
trait LocationTrait
{
	...
	public function getLocation()
	{
		...
		if (!empty($cityId)) {
			...
			if (!empty($cityName)) {
				...
				$this->fixTheLocationEntity($city, $admin);
	...
	public function getAdmin($adminName = null)
	{
		...
		if (in_array($adminType, ['1', '2'])) {
			...
			return cache()->remember($cacheId, $this->cacheExpiration, function () use ($adminModel, $adminName, $isAdminCode) {
				$admin = $adminModel::currentCountry();
				if ($isAdminCode) {
	...
	public function getAdminDivisionByNameAndGetItsPopularCity($adminName, bool $countryPopularCityAsFallback = true)
	{
		...
		// Get Admins 2
		if (!empty($admin1)) {
			$admins2 = SubAdmin2::currentCountry()->where('subadmin1_code', $admin1->code)->orderBy('name')->get(['code']);
		} else {
	...
	/**
	 * Check if the admin name starts by 2 letters following by a dot (.) and with other characters
	 *
	 ...
	 * @return void
	 */
	private function fixTheLocationEntity(City|null|SubAdmin1|SubAdmin2 &$city, SubAdmin1|SubAdmin2|null|City &$admin): void
	{
