...
class MessagesController extends AccountBaseController
{
    ...
    public function index()
    {
        ...
        // Check for API errors or a null result
        $success = data_get($data, 'success', false);
        ...
        if ($success && $apiResult !== null) {
            // Process when the API call is successful and result is not null
            ...
            // Process when the API call fails or result is null
    
            if (request()->ajax()) {
                return response()->json(['error' => 'Failed to retrieve data.'], 400);
            }
    
            return view('account.messenger.index')->withErrors(['error' => 'Failed to retrieve data.']);
        }
    }
