...
class CreateController extends FrontController
{
	...
	public function commonQueries()
	{
        ...
        // Check if $keywords is empty, and if so, provide a default value
        if (empty($keywords)) {
            $keywords = $title;
        }
        // dd($keywords);
	...
	public function getPostStep(Request $request)
	{
	    ...
	    $sessionCateSlug = session()->get('_previous.url');
		$segments = explode('/', $sessionCateSlug);
		$lastSegment = end($segments);
		
		$categoryslugid = Category::where('slug', $lastSegment)->first();
		...
		return appView('post.createOrEdit.multiSteps.create', compact('postInput', 'categoryslugid'));
	}
	...
	public function getPicturesStep(Request $request)
	{
		...
		$packageIds = intval($packageId);

		$picturesLimit = 0;

		if ((($packageId == null || $packageId == 1) && $picturesLimit = 20) ||
			($packageId == 2 && $picturesLimit = 30) ||
			($packageId == 3 && $picturesLimit = 40) ||
			($packageId == 4 && $picturesLimit = 50)
		) 

		$totalAllowed = $picturesLimit;
		if(is_array($picturesInput)){
		  $totalCount = count($picturesInput);
		}else{
		    $totalCount = 0;
		}
		$remainingCount = $totalAllowed - $totalCount;
		if ($remainingCount < 0) {
			$remainingCount = 0;
		}
		
		return appView('post.createOrEdit.multiSteps.photos.create', compact('picturesInput', 'remainingCount', 'totalCount'));
	}
