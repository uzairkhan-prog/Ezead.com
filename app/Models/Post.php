...
class Post extends BaseModel implements Feedable
{
	...
	*/
	...
	public function getPictureHtml(): string
	{
		...
		$style = ' style="width:100%;height:90px;"';
		// Get first picture
		$out = '';
		if ($this->pictures->count() > 0) {
			$url = dmUrl($this->country_code, UrlGen::postPath($this));
			foreach ($this->pictures as $picture) {
                // Hide Custom Cloud Flare R2 (thumb-120x90-) Admin Panel
				// $basename = basename($picture->filename);
				// $thumbBasename = 'thumb-120x90-' . $basename;
				// $directory = dirname($picture->filename);
				// $thumbSmall = $directory . '/' . $thumbBasename;
				// $out .= '<img src="' . imgUrl($thumbSmall) . '" data-bs-toggle="tooltip" title="' . $this->title . '"' . $style . ' class="img-rounded">';
	...
	// Custom Admin Listing Page Location
	...
	// 	$new_city = Region::where('id', $this->new_city_id)->first();
	// 	$neighbour = Region::where('id', $this->neighbour_id)->first();
	// 	if (!empty($neighbour)) {
	...
	// Custom Admin Listing Page Location (Modify Code)
	public function getRegionHtml()
	{
		$region = null;
		if (!empty($this->neighbour_id)) {
			$region = Region::find($this->neighbour_id);
		} elseif (!empty($this->new_city_id)) {
			$region = Region::find($this->new_city_id);
		} elseif (!empty($this->region_id)) {
			$region = Region::find($this->region_id);
		} elseif (!empty($this->province_id)) {
			$region = Region::find($this->province_id);
		} elseif (!empty($this->country_code)) {
			$country = Country::where('code', $this->country_code)->first();
			if (!empty($country)) {
				if ($country->code == 'LP') {
					return '<i class="bi bi-geo-alt"></i> Canada';
				} else {
					$region = $country;
				}
			}
		}
		return $region ? '<i class="bi bi-geo-alt"></i>  ' . $region->name : null;
	}
	...
	*/
	...
	{
		return $this->belongsTo(User::class, 'user_id')->userDetail();
	}
}
