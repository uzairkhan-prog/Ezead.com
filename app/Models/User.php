	...
	protected function photoUrl(): Attribute
	{
		return Attribute::make(
			get: function ($value) {
				...
				// Custom Add Cloud Flare R2 Get Profile Photo
				$userDetail = $this->userDetail()->first();
                if ($userDetail && $userDetail->profile_photo) {
                    $userPhotoUrl = env('CLOUDFLARE_R2_URL') . '/assets/images/' . $userDetail->profile_photo;
                }
	...
	protected function pIsOnline(): Attribute
	{
		return Attribute::make(
			get: function ($value) {
				...
				$isOnline = (!empty($this->getRawOriginal('last_activity'))
