...
trait KeywordFilter
{
	...
	protected function applyKeywordFilter()
	{
		...
		if (request()->get('matching') == '1') {
			if (request()->get('search_title') && request()->get('search_description')) {
				return $this->posts->where('title', 'LIKE', "%$keywords%")
					->orWhere('posts.description', 'LIKE', "%$keywords%");
			} elseif (request()->get('search_title')) {
				return $this->posts->where('title', 'LIKE', "%$keywords%");
			} elseif (request()->get('search_description')) {
				return $this->posts->where('posts.description', 'LIKE', "%$keywords%");
			} else {
				return $this->posts->where('title', 'LIKE', "%$keywords%")
					->orWhere('posts.description', 'LIKE', "%$keywords%");
			}
		} elseif (request()->get('matching') == '2') {
			if (request()->get('search_title') && request()->get('search_description')) {
				return $this->posts->where('title', $keywords)
					->orWhere('posts.description', $keywords);
			} elseif (request()->get('search_title')) {
				return $this->posts->where('title', $keywords);
			} elseif (request()->get('search_description')) {
				return $this->posts->where('posts.description', $keywords);
			} else {
				return $this->posts->where('title', $keywords)
					->orWhere('posts.description', $keywords);
			}
		} elseif (request()->get('matching') == '3') {
			return $this->posts->find($keywords);
		}
		// custom Query end
