...
function resize($disk, ?string $filePath, ?string $preConfigSize = 'big', bool $webpFormat = false): string
{
	...
	// if (!\Illuminate\Support\Facades\Storage::disk('r2')->exists('ezead-com/public/storage/' . $thumbFilePath)) {
	if (!$disk->exists($thumbFilePath)) {
		...
		// Get file path R2 
		$filePath_R2 = env('CLOUDFLARE_R2_STORE') . $thumbFilePath;
		// Store the image on R2 disk
		\Illuminate\Support\Facades\Storage::disk('r2')->put($filePath_R2, $image->stream()->__toString());
